...
def caller_account_id() -> str:
    """The AWS account id of the ambient credentials — used to build the
    ``iam:PassRole`` ARN for the orchestrator role's policy."""
    if _DRY_RUN:
        _log("sts get-caller-identity (account id)")
        return "000000000000"
    return _client("sts").get_caller_identity()["Account"]


def default_subnet_ids() -> list[str]:
    """Default-VPC subnet ids (one per AZ) for the ASG's VPCZoneIdentifier. The
    durable orchestrator only needs outbound HTTPS, so a public default subnet is
    fine — no dedicated networking to stand up."""
    if _DRY_RUN:
        _log("describe-subnets (default-for-az)")
        return ["subnet-DRYRUN"]
    r = _client("ec2").describe_subnets(Filters=[{"Name": "default-for-az", "Values": ["true"]}])
...
def ensure_orchestrator_profile(
    role_name: str, profile_name: str, bucket: str, account_id: str
) -> None:
    """Create the durable orchestrator's role + instance profile. Unlike the
    worker role (S3-only), this carries CONTROLLER permissions — it runs the
    supervisor, so it launches/terminates GPU boxes, reads/writes the run's S3
    objects (including DeleteObject for epoch.json in a whole-group restart),
    passes the worker role to EC2, sends the SSM preemption signal, and
    self-scales its OWN ASG to 0 on completion. Mirrors
    docs/iam/orchestrator-policy.json. Idempotent."""
    import json

    _log(f"create orchestrator IAM role {role_name} + instance profile {profile_name}")
    if _DRY_RUN:
        return
    iam = _client("iam")
    assume = {
    ...
    policy = {
        ...
        "Statement": [
            ...
            {
                "Sid": "PassWorkerRoleToEc2Only",
                "Effect": "Allow",
                "Action": "iam:PassRole",
                "Resource": f"arn:aws:iam::{account_id}:role/spot-train-role",
                "Condition": {"StringEquals": {"iam:PassedToService": "ec2.amazonaws.com"}},
            },
            {
                "Sid": "SsmRunCommandForPreemption",
                "Effect": "Allow",
    ...
    iam.put_role_policy(
        RoleName=role_name, PolicyName="spot-orch", PolicyDocument=json.dumps(policy)
    )
    _ignore_exists(lambda: iam.create_instance_profile(InstanceProfileName=profile_name))
    attached = [
        r["RoleName"]
...
# --------------------------------------------------------------------------- #
# Mutating: launch template + Auto Scaling Group (durable orchestrator)
# --------------------------------------------------------------------------- #
def ensure_launch_template(
    ...
    *,
    ...
    """Create a launch template describing the durable orchestrator box (mirrors
    ``launch``'s kwargs). The ASG launches from this. Returns the template name.
    Idempotent: on a re-run it adds a new default version rather than failing."""
    import base64

    _log(f"CreateLaunchTemplate {name} type={instance_type} ami={ami_id}")
    if _DRY_RUN:
        return name
    lt_data: dict[str, Any] = {
        ...
        "InstanceType": instance_type,
        "IamInstanceProfile": {"Name": profile_name},
        # Public IP + SG via the interface (same shape as launch()) so the box has
        # outbound HTTPS to EC2/S3/IAM/SSM without a NAT gateway.
        "NetworkInterfaces": [
            {
                "DeviceIndex": 0,
                "AssociatePublicIpAddress": True,
    ...
    ec2 = _client("ec2")
    try:
        ec2.create_launch_template(LaunchTemplateName=name, LaunchTemplateData=lt_data)
    except Exception as e:  # noqa: BLE001 — boto ClientError
        if "AlreadyExistsException" not in str(e):
            raise
        v = ec2.create_launch_template_version(LaunchTemplateName=name, LaunchTemplateData=lt_data)[
            "LaunchTemplateVersion"
        ]["VersionNumber"]
        ec2.modify_launch_template(LaunchTemplateName=name, DefaultVersion=str(v))
    return name
...
def ensure_auto_scaling_group(
    ...
    *,
    ...
    """Create the desired=1 ASG that keeps the orchestrator box alive for the
    job's duration (it self-heals a box that dies mid-job). Idempotent: updates
    capacity if the group already exists."""
    _log(f"CreateAutoScalingGroup {name} lt={launch_template_name} desired={desired}")
    if _DRY_RUN:
        return
    ...
    kwargs: dict[str, Any] = {
        ...
        "Tags": [
            {"Key": k, "Value": v, "PropagateAtLaunch": True} for k, v in (tags or {}).items()
        ],
    }
    try:
        asg.create_auto_scaling_group(**kwargs)
    except Exception as e:  # noqa: BLE001 — boto ClientError
        if "AlreadyExists" not in str(e):
            raise
        set_asg_capacity(name, min_size=min_size, max_size=max_size, desired=desired)


def set_asg_capacity(name: str, *, min_size: int, max_size: int, desired: int) -> None:
    """Resize the ASG — the box calls this to self-scale to 0 on completion, and
    teardown uses it before deleting the group."""
    _log(f"UpdateAutoScalingGroup {name} min={min_size} max={max_size} desired={desired}")
    if _DRY_RUN:
        return
    _client("autoscaling").update_auto_scaling_group(
        AutoScalingGroupName=name, MinSize=min_size, MaxSize=max_size, DesiredCapacity=desired
    )


def describe_asg(name: str) -> dict | None:
    """The ASG's current state ({name, desired, instances:[{id,state,health}]}),
    or None if it doesn't exist. Read-only — for ``remote-status`` and to find the
    live instance the fault injector terminates."""
    if _DRY_RUN:
        _log(f"describe-auto-scaling-groups {name}")
        return None
    r = _client("autoscaling").describe_auto_scaling_groups(AutoScalingGroupNames=[name])
...
def delete_asg(name: str, force: bool = True) -> None:
    """Delete the ASG (ForceDelete terminates any remaining instance). Idempotent
    — a missing group is not an error."""
    _log(f"DeleteAutoScalingGroup {name} force={force}")
    if _DRY_RUN:
        return
    try:
        _client("autoscaling").delete_auto_scaling_group(
            AutoScalingGroupName=name, ForceDelete=force
        )
    except Exception as e:  # noqa: BLE001 — boto ClientError; missing group is fine
        if "not found" not in str(e).lower() and "AlreadyExists" not in str(e):
            raise


def delete_launch_template(name: str) -> None:
    """Delete the launch template. Idempotent — a missing template is not an error."""
    _log(f"DeleteLaunchTemplate {name}")
    if _DRY_RUN:
        return
    try:
        _client("ec2").delete_launch_template(LaunchTemplateName=name)
    except Exception as e:  # noqa: BLE001 — boto ClientError; missing template is fine
